...
function animalNames(array) {
  zooAnimals.forEach((cur) => {
    array.push({ name: cur.animal_name, scientific: cur.scientific_name });
  });
...
function lowerCaseNames() {
  const allNames = zooAnimals.map((cur) => cur.animal_name.toLowerCase());
...
function lowPopulationAnimals() {
  const filteredAnimals = zooAnimals.filter((cur) => cur.population < 5);
...
function USApop() {
  const totalAnimals = zooAnimals.reduce((acc, current) => {
    acc += current.population;
